...
Write-Host "[VALIDATION] NLCI Validation" -ForegroundColor Cyan
...
function Test-Section {
    ...
    try {
        & $Test
        Write-Host "  [PASS] PASS`n" -ForegroundColor Green
    }
    catch {
        Write-Host "  [FAIL] FAIL: $_`n" -ForegroundColor Red
        $script:FailureCount++
    }
}
...
if ($script:FailureCount -eq 0) {
    ...
    Write-Host "[FAIL] Validation FAILED with $script:FailureCount error(s)" -ForegroundColor Red
    exit 1
}
