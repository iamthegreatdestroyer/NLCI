...
New-Item -ItemType Directory -Path (Join-Path $staging "extension") -Force | Out-Null
New-Item -ItemType Directory -Path (Join-Path $staging "extension\dist") -Force | Out-Null
New-Item -ItemType Directory -Path (Join-Path $staging "extension\resources") -Force | Out-Null

Write-Host "[1/4] Copying extension files..." -ForegroundColor Yellow
Copy-Item (Join-Path $ExtDir "package.json") (Join-Path $staging "extension\package.json") -Force
Copy-Item (Join-Path $ExtDir "README.md") (Join-Path $staging "extension\README.md") -Force
Copy-Item (Join-Path $ExtDir "LICENSE") (Join-Path $staging "extension\LICENSE") -Force
Copy-Item (Join-Path $ExtDir "dist\extension.js") (Join-Path $staging "extension\dist\extension.js") -Force
Copy-Item (Join-Path $ExtDir "resources\icon.svg") (Join-Path $staging "extension\resources\icon.svg") -Force
...
$contentTypes = '<?xml version="1.0" encoding="utf-8"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension=".json" ContentType="application/json"/><Default Extension=".js" ContentType="application/javascript"/><Default Extension=".md" ContentType="text/markdown"/><Default Extension=".svg" ContentType="image/svg+xml"/><Default Extension=".txt" ContentType="text/plain"/><Override FileName="/extension.vsixmanifest" ContentType="text/xml"/></Types>'
